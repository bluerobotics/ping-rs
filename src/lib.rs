...
impl From<decoder::ParseError> for PingError {
    fn from(other: decoder::ParseError) -> Self {
        Self(other.to_string())
    }
}
...
impl<D: Read + Write> PingDevice<D> {
    ...
    pub fn request(
        ...
        self.decoder.reset();
        Err(PingError("Missing bytes, can't complete".to_string()))
    }
    ...
    pub fn get_protocol_version(&mut self) -> Result<common::ProtocolVersionStruct, PingError> {
        ...
        match Messages::try_from(&answer) {
            ...
            _ => Err(PingError("Not expected structure".to_string())),
        }
    }
